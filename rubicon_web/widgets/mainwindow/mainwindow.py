...
class MainWindow(QMainWindow, Ui_MainWindow):
    def __init__(self, parent, configutils, log_function, app_dir, *args, **kwargs):
        ...
        self.relative_to_abs_path = lambda file_path: os.path.join(
            self.app_dir, file_path
        )
        self.nt_to_posix_path = lambda file_path: "".join(
            str(PurePosixPath(PureWindowsPath(file_path))).split("\\")[1:]
        )
    ...
    def closeEvent(self, event):
        ...
        with open(self.configutils.last_session_path, "w") as last_session_file:
            ...
            self.log(
                f"Saved current session to {self.configutils.last_session_path}",
                "SUCCESS",
                "mainwindow.py",
            )
